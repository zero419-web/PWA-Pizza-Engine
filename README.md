...
- **Canale IPC PostMessage Protetto:** Implementazione di un ascoltatore formale per l'evento `message` deputato alla ricezione sicura dei comandi dal frontend, all'attivazione dei cicli di Smart Sync e al transito in RAM della chiave di cifratura non esportabile, isolata tramite l'algoritmo di clonazione strutturata della Structured Clone API.
